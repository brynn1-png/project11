...
- Active and archived product tables paginate the already-loaded, filtered catalog locally. They default to 25 rows, offer 10, 25, 50, and 100-row sizes, and reset to the first page when search, category, catalog mode, or page size changes.
...
- Products separates active and archived records. Administrators and managers may archive a product with a reason and restore it later. Archiving a product with remaining stock requires an explicit warning acknowledgement, records an inventory write-off for every affected batch, and reduces those batches to zero before the product is archived; pending resellable returns must still be reviewed first. Administrators alone may permanently delete an archived product after confirming its product code. Permanent deletion is an atomic database purge: related return records, sale lines, allocations, costs, receiving batches, adjustments, barcode aliases, and the product are removed; affected receipt totals are recalculated and receipts left without items are deleted.
