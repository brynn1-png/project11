...
export function ProductManagementView({ startCreating, canManage, canArchive, canReceive, notify, onReceive }: { startCreating: boolean; canManage: boolean; canArchive: boolean; canReceive: boolean; notify: (message: string) => void; onReceive: (productId: string) => void }) {
  ...
  function openArchivedProducts() { setCatalogMode("archived"); loadArchivedProducts(); }
  ...
  if (editing) {
    ...
    return <form className="panel overflow-hidden" onSubmit={save}>
      <div className="flex flex-col gap-3 border-b border-[var(--border)] p-5 sm:flex-row sm:items-start sm:justify-between sm:p-6"><div><p className="text-xs font-bold uppercase tracking-[0.08em] text-[var(--accent)]">{isNew ? "New product" : editing.id}</p><h2 className="mt-1 text-xl font-bold">{isNew ? "Register a product" : `Edit ${editing.name}`}</h2><p className="mt-1 text-sm text-[var(--muted-foreground)]">{isNew ? "Register the product and optionally add its opening stock in one step." : "Update the product details used throughout inventory and sales."}</p></div><Button type="button" variant="ghost" onClick={() => setEditing(null)}>Back to products</Button></div>
      <div className="grid gap-8 p-5 sm:p-6 lg:grid-cols-2">
        <fieldset className="grid content-start gap-4"><legend className="mb-3 text-base font-bold">Product details</legend>
          ...
          <div><label className="field-label" htmlFor="stock-unit">Unit of measure</label><Input id="stock-unit" placeholder="box, kilo, piece" value={form.stockUnit} onChange={(event) => set("stockUnit", event.target.value)} required maxLength={24} /><p className="mt-1.5 text-xs text-[var(--muted-foreground)]">How this product is counted during sales and stock receiving.</p></div>
        </fieldset>
        <fieldset className="grid content-start gap-4"><legend className="mb-3 text-base font-bold">Sales and tracking</legend>
          ...
          {isNew && canReceive && <div className="rounded-xl border border-[var(--border)] bg-[var(--muted)]/35 p-4">
            ...
            {addInitialStock && <div className="mt-4 grid gap-4 border-t border-[var(--border)] pt-4 sm:grid-cols-2">
              <div><label className="field-label" htmlFor="initial-quantity">Initial quantity</label><Input id="initial-quantity" type="number" min="1" max="1000000" step="1" value={initialStock.quantity} onChange={(event) => setInitialStock((current) => ({ ...current, quantity: event.target.value }))} required /><p className="mt-1.5 text-xs text-[var(--muted-foreground)]">Number of {form.stockUnit.trim() || "units"} received.</p></div>
          ...
          {form.barcodeMode === "manufacturer" ? <div><label className="field-label" htmlFor="product-barcode">Barcode</label><div className="relative"><Barcode className="absolute left-3.5 top-3.5 text-[var(--muted-foreground)]" size={18} /><Input id="product-barcode" className="pl-10 font-mono" value={form.barcode} onChange={(event) => set("barcode", event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); event.stopPropagation(); } }} aria-describedby="product-barcode-help" required minLength={4} maxLength={64} /></div><p id="product-barcode-help" className="mt-1.5 text-xs text-[var(--muted-foreground)]">{isNew ? "Scanning fills this field without saving. Review the details, then select Register product." : "Scanning fills this field without saving. The old barcode remains reserved as an inactive alias after you save."}</p></div> : <BarcodeGenerationPreview productName={form.name} />}
...
function ArchivedProductsPanel({ products, loading, canRestore, pending, onRestore }: { products: ArchivedProduct[]; loading: boolean; canRestore: boolean; pending: boolean; onRestore: (product: ArchivedProduct) => void }) {
