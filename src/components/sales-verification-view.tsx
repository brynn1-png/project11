...
export function SalesVerificationView({ notify }: { notify: (message: string) => void }) {
  ...
  return <div className="grid gap-5">
    ...
    <Card className="border-[var(--border)] bg-[var(--surface)] shadow-none"><CardHeader><CardTitle>Business days</CardTitle><CardDescription>Review the recorded sales and returns before locking a day as verified.</CardDescription></CardHeader><CardContent className="flex flex-col gap-3">{days.length === 0 ? <p className="py-8 text-center text-sm text-[var(--muted-foreground)]">A business day will open automatically when the first sale is confirmed.</p> : days.map((day) => { const hasPendingReturn = returns.some((item) => item.businessDayId === day.id); return <div key={day.id} className="grid gap-4 rounded-xl border border-[var(--border)] p-4 lg:grid-cols-[1fr_repeat(4,110px)_auto] lg:items-center"><div><p className="font-semibold">{date(day.businessDate)}</p><Badge variant={day.status === "verified" ? "default" : "outline"} className="mt-2">{day.status.replace("_", " ")}</Badge></div><Metric label="Sales" value={day.saleCount} /><Metric label="Items" value={day.itemCount} /><Metric label="Returns" value={day.returnCount} /><Metric label="Gross" value={peso(day.grossTotal)} /><div>{day.status === "open" ? <Button disabled={isPending} onClick={() => updateDay(day.id, "submit")}><Clock data-icon="inline-start" />Submit</Button> : day.status === "pending_review" ? <Button disabled={isPending || hasPendingReturn} onClick={() => updateDay(day.id, "verify")} title={hasPendingReturn ? "Review this day’s pending returns first." : undefined}><CheckCircle data-icon="inline-start" />Verify</Button> : <span className="text-sm font-semibold text-[var(--accent)]">Verified</span>}</div></div>; })}</CardContent></Card>
